...
print('This program allows for the storage and retrieval of a private key from a given image.')
print('-----------------------------------')
print('Do you want to:\nConceal a private key? (Press "a")\nReveal a private key? (Press "b")')
print('-----------------------------------')
while True:
...
if user_input == 'a':
 while True:
  ...
  privkey = raw_input('Please input your hexadecimal private key:\n')
  confirm = raw_input('Confirm that your private key is "' + privkey + '"?[y/n]\n')
  if confirm == 'y':
   break
   
 
 print('Generating QR...')
 QR = pyqrcode.create(privkey)
 ...
 image_to_conceal = Image.open('[[image name and extension]]')
 width, height = image_to_conceal.size
 conceal_points = list(image_to_conceal.getdata())
 cut_image = conceal_points[:2025]
 balanced_image = list()
 for i in cut_image:
 ...
 completed_image = image_with_qr + conceal_points[8100:]
...
if user_input == 'b':
 ...
 honeybadger_getpoints = honeybadger_read[:2025]
 ...
 QRnew = Image.new('RGB', (45,45))
 QRnew.putdata(newconstruct)
 QRnew = QRnew.resize((90,90), Image.ANTIALIAS)
